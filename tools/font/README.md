...
- `germandbls`：U+00DF ß。將原始 `f` 的上行筆勢／莖部與原始 `s` 的終筆做受控縮放、裁切，加入同筆寬的圓端過渡筆後 boolean union，輸出單一連續外輪廓；不是 B、Greek β、`fs` 或兩個未處理字母。
- `uni1E9E`：U+1E9E ẞ。由原始 `P` 的大寫莖與上 bowl、原始 `S` 下筆，加上斜向 transition／shoulder strokes 組成；保留淺開口並移除 boolean 微小 sliver contours，維持 cap-height 手寫語言、連續筆勢及與普通 B 的區別。
