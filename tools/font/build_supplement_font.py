...
def build_oe_ligature(font: TTFont, codepoint: int, glyph_name: str, o_codepoint: int, e_codepoint: int) -> None:
    """Derive one OE ligature solely from its source-native O/o and E/e.

    The E/e tucks into the O/o by ten percent of the measured O/o ink width. Both
    source outlines retain their native scale and baseline; the final advance
    is calculated from the resulting ink bounds plus the source E/e bearing.
    """
    cmap = font.getBestCmap()
    o_name = cmap.get(o_codepoint)
    e_name = cmap.get(e_codepoint)
    if o_name is None or e_name is None:
        fail(f"Source font lacks O/E inputs required to derive U+{codepoint:04X}")

    o_bounds = glyph_bounds(font, o_name)
    e_bounds = glyph_bounds(font, e_name)
    o_advance, o_lsb = font["hmtx"].metrics[o_name]
    e_advance, _ = font["hmtx"].metrics[e_name]
    o_rsb = o_advance - o_bounds[2]
    e_rsb = e_advance - e_bounds[2]
    if min(o_lsb, o_rsb, e_rsb) < 0:
        fail("Source o/e have unexpected negative bearings")

    o_ink_width = o_bounds[2] - o_bounds[0]
    overlap = max(1, round(o_ink_width * OE_OVERLAP_RATIO))
    e_dx = o_bounds[2] - e_bounds[0] - overlap

    pen = TTGlyphPen(font.getGlyphSet())
    pen.addComponent(o_name, (1, 0, 0, 1, 0, 0))
    pen.addComponent(e_name, (1, 0, 0, 1, e_dx, 0))
    glyph = pen.glyph()
    glyph.recalcBounds(font["glyf"])
    advance = glyph.xMax + e_rsb
    install_glyph(font, glyph_name, glyph, advance, glyph.xMin, o_name)
    add_unicode_mapping(font, codepoint, glyph_name)


def build_oe_ligatures(font: TTFont) -> None:
    """Add native French uppercase and lowercase OE ligatures."""
    build_oe_ligature(font, 0x0152, "OE", 0x004F, 0x0045)
    build_oe_ligature(font, 0x0153, "oe", 0x006F, 0x0065)


def build_french_guillemets(font: TTFont) -> None:
    """Nest two authorized source chevrons at the Latin visual midline.

    The SHA-pinned original has neither U+00AB/U+00BB nor U+2039/U+203A.
    Reuse its complete less/greater drawings as components, without redrawing
    any points. The source greater is rotated/compressed to open its flat upper arm;
    both pairs occupy approximately (32, 163, 295, 417) in a 327-unit cell.
    Separate component origins tighten the pair without contour intersection.
    All scales are exact binary fractions in TrueType's F2Dot14 format.
    """
    cmap = font.getBestCmap()
    if any(cp in cmap for cp in (0x00AB, 0x00BB, 0x2039, 0x203A)):
        fail("Source quotation coverage changed; re-inspect the source priority")
    recipes = (
        (0x00AB, "guillemotleft", 0x003C, (0.75, 0, 0, 0.75), -6, 43, 112),
        (0x00BB, "guillemotright", 0x003E, (0.453125, -0.4375, 0.2109375, 0.9375), -30, 7, 121),
    )
    for cp, name, source_cp, linear, dx, dy, step in recipes:
        source_name = cmap.get(source_cp)
        if source_name is None:
            fail(f"Missing authorized source U+{source_cp:04X}")
        pen = TTGlyphPen(font.getGlyphSet())
        pen.addComponent(source_name, (*linear, dx, dy))
        pen.addComponent(source_name, (*linear, dx + step, dy))
        glyph = pen.glyph()
        glyph.recalcBounds(font["glyf"])
        install_glyph(font, name, glyph, 327, glyph.xMin, source_name)
        add_unicode_mapping(font, cp, name)
...
def validate_inputs(manifest: dict) -> None:
    ...
    from kana_sources.hiragana_master_v2 import REFERENCES
    for reference, expected_hash in REFERENCES.items():
        master = TOOLS_DIR / "references" / reference
        if not master.is_file() or sha256(master) != expected_hash:
            fail(f"Maintainer Hiragana Master v2 reference is missing or changed: {reference}")
...
"""
    with OUTPUT_MODIFICATIONS.open("w", encoding="utf-8", newline="\n") as stream:
        stream.write(content)


def write_ofl() -> None:
    lines = SOURCE_LICENSE.read_text(encoding="utf-8").splitlines()
    with OUTPUT_OFL.open("w", encoding="utf-8", newline="\n") as stream:
...
def main() -> int:
    try:
        ...
        for codepoint in (0x00BF, 0x00C7, 0x00E7, 0x0327, 0x00A8, 0x00DF, 0x0152, 0x0153, 0x1E9E):
