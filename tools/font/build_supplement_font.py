...
BUILD_TIMESTAMP = int((datetime(2026, 8, 25, tzinfo=timezone.utc) - MAC_EPOCH).total_seconds())
...
- Optical normalization（Version 1.014）：在 1.013 手寫結構外層逐字套用等比例 scale／translation，不替換筆畫拓撲；全部小平假名由 normalization 後的大字以 0.72 比例衍生
...
- Japanese special glyphs：Version 1.014 的 U+61D0 `懐` 結合原生 `懷` 上／左結構與等比例縮放定位的原生 `衣` 下部；U+3005 `々` 使用 project-local center-lines 並將右側缺口連成連續筆勢；U+5915 `夕` 完全不改。U+6C17 `気` 與 U+4ED8 `付` 仍只建立保留原來源輪廓的垂直 optical transform copy
