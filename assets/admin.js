...
function adminSongCard(song) {
  ...
  const read = node("a", "button secondary", "播放與閱讀"); read.href = `./song.html?id=${encodeURIComponent(song.id)}`; const read = node("a", "button secondary", "播放與閱讀"); read.href = `./song.html?id=${encodeURIComponent(song.id)}`;
